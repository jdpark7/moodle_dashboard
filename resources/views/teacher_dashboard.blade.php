...
<div class="fade-in space-y-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
            <h2 class="text-2xl font-bold text-white font-sans">{{ session('fullname') }} 교수님 👨‍🏫</h2>
    ...
    <div class="glass p-6 rounded-3xl border border-slate-850 bg-gradient-to-r from-slate-950/60 to-indigo-950/10">
        ...
        <p class="text-sm text-slate-400 leading-relaxed max-w-4xl">{{ strip_tags($selected_course['summary'] ?? '이 강좌에 등록된 요약 내용이 없습니다.') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="glass p-6 rounded-2xl border border-slate-800 flex items-center gap-5">
            ...
            <div>
                <span class="block text-2xl font-bold text-white">{{ $total_students }}명</span>
        ...
        <div class="glass p-6 rounded-2xl border border-slate-800 flex items-center gap-5">
            ...
            <div>
                <span class="block text-2xl font-bold text-white">{{ $pending_grades_count }}건</span>
                    ...
                    <div class="space-y-3.5">
                        @foreach ($risk_students as $s)
                            <div class="p-3.5 rounded-xl bg-rose-500/5 border border-rose-500/15 flex items-center gap-3.5">
                                ...
                                <button type="button" onclick="openStudentModal({{ $s['id'] }}, '{{ $s['fullname'] }}', '{{ $s['email'] }}', '{{ $s['profileimageurl'] }}', {{ $s['progress'] }}, {{ $s['grade'] }}, '{{ addslashes($s['feedback']) }}', '{{ $s['is_risk_computed'] }}')" class="w-8 h-8 rounded-lg flex items-center justify-center border border-rose-500/20 text-rose-400 hover:bg-rose-500/10 transition-colors" title="진단 피드백">
                        ...
                        @foreach ($assignments_summary as $a)
                            <tr class="hover:bg-slate-900/10 transition-colors">
                                ...
                                <td class="py-4 px-5 text-center font-bold">
                                    <span>{{ $a['submitted_text'] }}명</span>
                                    <span class="text-slate-500 font-normal">({{ $a['submission_rate'] }}%)</span>
                                </td>
                                <td class="py-4 px-5 text-center font-bold">
                                    @if ($a['is_pending'])
                                        <span class="text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded">{{ $a['graded_text'] }} (채점대기)</span>
                                    @else
                                        <span class="text-green-400 bg-green-500/10 border border-green-500/20 px-2 py-0.5 rounded">{{ $a['graded_text'] }} (완료)</span>
                                    @endif
                                </td>
                                <td class="py-4 px-5 text-center font-bold text-brand-400">
                                    {{ $a['avg_score'] }} / {{ $a['max_score'] }}점
                                </td>
                                <td class="py-4 px-5 text-center">
