<?php $__env->startSection('title', '나의 수강 대시보드'); ?>
...
<div class="fade-in space-y-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-white font-sans">안녕하세요, <?php echo e(session('fullname')); ?> 학생 👋</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="glass p-6 rounded-2xl border border-slate-800 flex items-center gap-5">
            ...
            <div>
                <span class="block text-2xl font-bold text-white"><?php echo e($total_courses); ?></span>
        ...
        <div class="glass p-6 rounded-2xl border border-slate-800 flex items-center gap-5">
            ...
            <div>
                <span class="block text-2xl font-bold text-white"><?php echo e($pending_count); ?>개</span>
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <?php if(!empty($enrolled_courses)): ?>
                        <?php $__currentLoopData = $enrolled_courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="glass p-6 rounded-2xl border border-slate-850 flex flex-col justify-between glass-card-hover min-h-[200px]">
                                <div>
                                    <div class="flex items-center justify-between mb-3">
                                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-brand-500/15 text-brand-400 border border-brand-500/20 uppercase tracking-wider"><?php echo e($course['shortname']); ?></span>
                                        <span class="text-xs font-bold text-slate-400">ID: <?php echo e($course['id']); ?></span>
                                    </div>
                                    <h4 class="text-base font-bold text-white leading-tight mb-2 hover:text-indigo-300 transition-colors"><?php echo e($course['fullname']); ?></h4>
                                    <p class="text-xs text-slate-400 line-clamp-3 mb-4"><?php echo e(strip_tags($course['summary'] ?? '이 강좌에 대한 설명이 등록되지 않았습니다.')); ?></p>
                                </div>

                                <div class="space-y-2">
                                    <div class="flex items-center justify-between text-xs">
                                        <span class="text-slate-500">진행도 (이수율)</span>
                                        <span class="font-bold text-brand-400"><?php echo e($course['progress']); ?>%</span>
                                    </div>
                                    <div class="w-full bg-slate-950/80 rounded-full h-2 overflow-hidden border border-slate-800/40">
                                        <div class="bg-gradient-to-r from-brand-500 to-indigo-500 h-full rounded-full transition-all duration-300" style="width: <?php echo e($course['progress']); ?>%"></div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="space-y-4 pt-4">
                ...
                <div class="glass rounded-2xl border border-slate-800 overflow-hidden divide-y divide-slate-850">
                    <?php if(!empty($catalog_courses)): ?>
                        <?php $__currentLoopData = $catalog_courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-900/10 transition-colors">
                                <div class="flex-1 space-y-1">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] font-bold px-2 py-0.5 rounded-full bg-slate-800 text-slate-300 uppercase tracking-wider"><?php echo e($course['shortname']); ?></span>
                                        <span class="text-xs font-semibold text-slate-500">ID: <?php echo e($course['id']); ?></span>
                                    </div>
                                    <h4 class="text-sm font-bold text-white"><?php echo e($course['fullname']); ?></h4>
                                    <p class="text-xs text-slate-400 line-clamp-2"><?php echo e(strip_tags($course['summary'] ?? '상세 설명 없음')); ?></p>
                                </div>
                                <button type="button" onclick="enrollCourse('<?php echo e($course['id']); ?>')" class="px-4 py-2 text-xs font-bold rounded-xl bg-brand-600 hover:bg-brand-500 text-white transition-all shadow-md shrink-0 self-start md:self-center">
                                    수강신청
                                </button>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                ...
                <div class="glass p-6 rounded-2xl border border-slate-800 space-y-4">
                    <?php if(!empty($history_courses)): ?>
                        <div class="relative pl-6 border-l border-slate-800 space-y-6">
                            <?php $__currentLoopData = $history_courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="relative">
                                    ...
                                    <div>
                                        <span class="text-[10px] font-bold text-indigo-400 bg-indigo-500/10 px-2 py-0.5 rounded-full uppercase tracking-wider"><?php echo e($course['shortname']); ?></span>
                                        <h4 class="text-sm font-bold text-white mt-1.5"><?php echo e($course['fullname']); ?></h4>
                                        <div class="flex items-center gap-4 text-[11px] text-slate-500 mt-1">
                                            <span>진행도: 100% (이수 완료)</span>
                                            <span>•</span>
                                            <span>ID: <?php echo e($course['id']); ?></span>
                                        </div>
                                    </div>
                                </div>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="sticky top-6 space-y-4">
                ...
                <div class="glass p-6 rounded-3xl border border-slate-800/80 shadow-xl space-y-4">
                    <?php if(!empty($pending_assignments)): ?>
                        ...
                        <div class="space-y-4">
                            <?php $__currentLoopData = $pending_assignments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="p-4 rounded-xl bg-slate-950/50 border border-slate-850 hover:border-slate-800 transition-colors flex flex-col justify-between gap-3">
                                    <div class="space-y-1">
                                        <span class="text-[9px] font-bold px-2 py-0.5 rounded-full bg-indigo-900/40 text-indigo-400 uppercase tracking-wider"><?php echo e($a['course_name']); ?></span>
                                        <h4 class="text-xs font-bold text-slate-200 leading-snug"><?php echo e($a['name']); ?></h4>
                                    </div>

                                    <div class="flex items-center justify-between mt-1.5">
                                        <span class="flex items-center text-[10px] font-bold <?php echo e($a['days_left'] == 0 ? 'text-red-400' : ($a['days_left'] < 3 ? 'text-amber-400' : 'text-slate-400')); ?>">
                                            <i data-lucide="clock" class="w-3.5 h-3.5 mr-1"></i>
                                            <?php echo e($a['remaining_text']); ?>
...
<script>
    function enrollCourse(courseId) {
        ...
        fetch("<?php echo e(route('student.enroll')); ?>", {
            method: 'POST',
            body: formData,
            headers: {
...
<?php echo $__env->make('moodledash::layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\USER\moodledashboard-laravel\Modules/MoodleDash/resources/views/student_dashboard.blade.php ENDPATH**/ ?>