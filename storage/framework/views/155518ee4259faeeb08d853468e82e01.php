...
<head>
    ...
    <title><?php echo $__env->yieldContent('title', 'Moodle LMS Dashboard'); ?> - Antigravity</title>
    <!-- Google Fonts -->
    ...
    <div id="toast-container" class="fixed top-6 right-6 z-50 flex flex-col gap-3 max-w-sm w-full pointer-events-none">
        <?php if(session('success')): ?>
            <div class="toast-item glass flex items-center p-4 rounded-xl border-l-4 border-l-green-500 pointer-events-auto shadow-2xl fade-in" role="alert">
                ...
                <div class="text-sm font-medium text-slate-100"><?php echo e(session('success')); ?></div>
        ...
        <?php if(session('warning')): ?>
            <div class="toast-item glass flex items-center p-4 rounded-xl border-l-4 border-l-amber-500 pointer-events-auto shadow-2xl fade-in" role="alert">
                ...
                <div class="text-sm font-medium text-slate-100"><?php echo e(session('warning')); ?></div>
                <button class="ml-auto text-slate-400 hover:text-slate-200" onclick="this.parentElement.remove()">
                    <i data-lucide="x" class="w-4 h-4"></i>
                </button>
            </div>
        ...
        <?php if(session('info')): ?>
            <div class="toast-item glass flex items-center p-4 rounded-xl border-l-4 border-l-indigo-500 pointer-events-auto shadow-2xl fade-in" role="alert">
                ...
                <div class="text-sm font-medium text-slate-100"><?php echo e(session('info')); ?></div>
                <button class="ml-auto text-slate-400 hover:text-slate-200" onclick="this.parentElement.remove()">
                    <i data-lucide="x" class="w-4 h-4"></i>
                </button>
            </div>
        ...
        <?php if($errors->any()): ?>
            <?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="toast-item glass flex items-center p-4 rounded-xl border-l-4 border-l-red-500 pointer-events-auto shadow-2xl fade-in" role="alert">
                    ...
                    <div class="text-sm font-medium text-slate-100"><?php echo e($error); ?></div>
                    <button class="ml-auto text-slate-400 hover:text-slate-200" onclick="this.parentElement.remove()">
                        <i data-lucide="x" class="w-4 h-4"></i>
                    </button>
                </div>
        ...
        <aside class="w-72 glass border-r border-slate-800/80 hidden md:flex flex-col shrink-0">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2">
                <?php if(session('role') === 'student'): ?>
                    <a href="<?php echo e(route('student.dashboard')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 <?php echo e(request()->routeIs('student.dashboard') ? 'bg-brand-500/10 text-brand-400 border border-brand-500/20 font-semibold' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
                        <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
                        <span>나의 수강 대시보드</span>
                    </a>
                <?php elseif(session('role') === 'teacher'): ?>
                    <a href="<?php echo e(route('teacher.dashboard')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 <?php echo e(request()->routeIs('teacher.dashboard') ? 'bg-brand-500/10 text-brand-400 border border-brand-500/20 font-semibold' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
                        <i data-lucide="trending-up" class="w-5 h-5"></i>
                        <span>학습 분석 대시보드</span>
                    </a>
                <?php endif; ?>
                
                <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 <?php echo e(request()->routeIs('profile.edit') ? 'bg-brand-500/10 text-brand-400 border border-brand-500/20 font-semibold' : 'text-slate-400 hover:bg-slate-800/40 hover:text-slate-200'); ?>">
            ...
            <div class="p-4 border-t border-slate-800/50">
                <div class="flex items-center gap-3 p-2 rounded-xl bg-slate-900/30 border border-slate-800/30">
                    <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-3 flex-1 min-w-0 group">
                        <img src="<?php echo e(session('userpictureurl')); ?>" alt="Profile" class="w-10 h-10 rounded-lg object-cover ring-2 ring-indigo-500/30 group-hover:ring-brand-500/50 transition-all bg-slate-800">
                        <div class="flex-1 min-w-0">
                            <h4 class="text-xs font-semibold text-slate-200 truncate group-hover:text-brand-400 transition-colors"><?php echo e(session('fullname')); ?></h4>
                            <p class="text-[10px] text-slate-500 uppercase tracking-wider font-semibold">
                                <?php echo e(session('role') === 'teacher' ? '교수자 (Teacher)' : '학생 (Student)'); ?>

                            </p>
                        </div>
                    </a>
                    <a href="<?php echo e(route('logout')); ?>" class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-colors" title="로그아웃">
        ...
        <main class="flex-1 flex flex-col min-w-0">
            ...
            <header class="h-16 px-6 glass border-b border-slate-800/50 flex items-center justify-between">
                ...
                <div class="hidden md:flex items-center gap-2 text-xs font-medium text-slate-400">
                    ...
                    <span class="px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 font-mono"><?php echo e(session('moodle_url', 'Not Connected')); ?></span>
                </div>

                <div class="flex items-center gap-4">
                    <?php if(session('is_demo')): ?>
                    ...
                    <?php if(session()->has('role')): ?>
                        <a href="<?php echo e(route('profile.edit')); ?>" class="md:hidden text-slate-400 hover:text-slate-200 mr-2" title="프로필 수정">
                            <i data-lucide="settings" class="w-5 h-5"></i>
                        </a>
                        <a href="<?php echo e(route('logout')); ?>" class="md:hidden text-slate-400 hover:text-slate-200" title="로그아웃">
...
<?php /**PATH C:\Users\USER\moodledashboard-laravel\Modules/MoodleDash/resources/views/layouts/app.blade.php ENDPATH**/ ?>